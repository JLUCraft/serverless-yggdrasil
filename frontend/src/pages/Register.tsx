...
import { api, allowedEmailDomains } from '../lib/api';
...
export default function Register() {
  ...
  async function handleSubmit(e: Event) {
    ...
    setLoading(true);
    try {
      const res = await api.auth.registerInitiate(username(), password(), email);
    ...
    <div class="min-h-screen w-full flex flex-col lg:flex-row bg-base-100">
      ...
      <div class="flex-1 flex items-center justify-center p-6 sm:p-10 animate-fade-in">
        <div class="w-full max-w-sm space-y-5">
          ...
          <div>
            <h2 class="text-2xl font-bold text-slate-800 tracking-tight">创建您的账号</h2>
            <p class="mt-1 text-sm text-slate-500">今天就开始使用 {siteConfig.shortName}</p>
          </div>

          <form onSubmit={handleSubmit} class="space-y-4">
            <div>
              <label class="block text-sm font-semibold text-slate-700 mb-1.5">用户名</label>
              <input
                type="text"
            ...
            <div>
              <label class="block text-sm font-semibold text-slate-700 mb-1.5">校园邮箱</label>
              <div class="flex">
                <input
                  type="text"
                  ...
                  required
                />
                <div class="flex items-center px-2.5 bg-slate-50 border border-slate-300 border-l-0 text-slate-500 text-sm select-none">@</div>
                <select
            ...
            <div>
              <label class="block text-sm font-semibold text-slate-700 mb-1.5">密码</label>
              <input
                type="password"
                class="input glass-input w-full focus:ring-2 focus:ring-primary/20 focus:outline-none"
                placeholder="至少 8 位"
            ...
            <div>
              <label class="block text-sm font-semibold text-slate-700 mb-1.5">确认密码</label>
              <input
                type="password"
                class="input glass-input w-full focus:ring-2 focus:ring-primary/20 focus:outline-none"
                placeholder="再次输入密码"
