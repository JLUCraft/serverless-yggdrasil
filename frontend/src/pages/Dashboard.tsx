...
import { api, type Profile, type Shape, type Texture, type TextureKind, type User } from '../lib/api';
  ...
  const userCard = () => (
    <div class="rounded bg-primary px-4 py-4 text-white">
      ...
      <div class="mt-3 flex flex-wrap gap-1.5 text-xs font-semibold uppercase">
        <span class="rounded-full bg-white/15 px-2.5 py-0.5">{member()?.role}</span>
        <span class="rounded-full bg-white/15 px-2.5 py-0.5">{member()?.status}</span>
      </div>
    </div>
  );
      ...
      <Show when={!loading() && member()}>
        ...
        <div class="space-y-5">
          ...
          <div class="flex items-center justify-between border-b border-slate-200 pb-4">
            <div>
              <h1 class="text-2xl font-bold tracking-tight text-slate-800">档案配置</h1>
              <p class="mt-0.5 text-sm text-slate-500">
                通行证 <span class="font-mono">{member()?.uuid}</span>
                {member()?.club && <span class="ml-2 text-slate-400">· {member()?.club}</span>}
              </p>
            </div>
          ...
          <Show when={showCreate()}>
            <div class="glass-panel p-5">
              <h2 class="mb-4 font-bold text-slate-800">新建档案</h2>
              <form onSubmit={createProfile} class="grid gap-4 sm:grid-cols-[1fr_1fr_auto]">
                <div>
                  <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-slate-500">档案名称</label>
                  <input
                ...
                <div>
                  <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-slate-500">角色模型</label>
                  <select
          ...
          <Show when={!profiles().length && !showCreate()}>
            <div class="rounded border border-dashed border-slate-300 bg-slate-50 px-6 py-16 text-center">
              <p class="text-sm text-slate-500">还没有任何档案</p>
              ...
              <div class="space-y-2">
                <For each={profiles()}>
                  {(entry) => {
                    ...
                    return (
                      <article
                        class={`glass-panel cursor-pointer overflow-hidden transition-all ${selected() ? 'ring-2 ring-primary/40' : 'hover:bg-slate-50'}`}
                        onClick={() => selectProfile(entry.id)}
                      >
                        <div class="flex items-center gap-4 p-4">
                          <div class="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded border border-slate-200 bg-slate-100">
                            <Show
                              when={entry.skin}
                              fallback={<span class="text-xl font-black text-slate-300">{entry.name.slice(0, 2).toUpperCase()}</span>}
                            >
                          ...
                          <div class="min-w-0 flex-1">
                            <div class="flex items-center justify-between gap-3">
                              <h3 class="truncate font-bold text-slate-800">{entry.name}</h3>
                              <span class="shrink-0 rounded bg-slate-100 px-2 py-0.5 font-mono text-xs text-slate-500">{entry.model}</span>
                            </div>
                            <div class="mt-2 flex gap-2">
                              <span class={`rounded px-2 py-1 text-xs font-semibold ${entry.skin ? 'bg-primary/10 text-primary' : 'bg-slate-100 text-slate-400'}`}>
                                皮肤 {entry.skin ? '✓' : '—'}
                              </span>
                              <span class={`rounded px-2 py-1 text-xs font-semibold ${entry.cape ? 'bg-primary/10 text-primary' : 'bg-slate-100 text-slate-400'}`}>
                                披风 {entry.cape ? '✓' : '—'}
                              </span>
                            </div>
                          </div>
                          <svg xmlns="http://www.w3.org/2000/svg" class={`h-4 w-4 shrink-0 transition-colors ${selected() ? 'text-primary' : 'text-slate-200'}`} viewBox="0 0 20 20" fill="currentColor">
              ...
              <Show
                ...
                fallback={
                  <div class="glass-panel flex flex-col items-center justify-center gap-2 px-6 py-14 text-center">
                    ...
                    <p class="text-sm font-semibold text-slate-600">选择一个档案</p>
                    <p class="text-xs text-slate-400">开始配置皮肤与披风</p>
                  </div>
                }
              >
                <div class="space-y-3">
                  ...
                  <div class="glass-panel p-4 space-y-4">
                    <div class="flex items-center justify-between">
                      <h3 class="font-bold text-slate-800">配置纹理</h3>
                      <span class="text-[11px] text-slate-400">从纹理库选择或上传</span>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-2">
                      {(['skin', 'cape'] as const).map((k) => (
                        <button
                          type="button"
                          class={`rounded border py-2 text-sm font-semibold transition-colors ${kind() === k ? 'border-primary/25 bg-primary/10 text-primary' : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50'}`}
                          onClick={() => {
                            setKind(k);
                            setChosen('');
                          }}
                    ...
                    <div>
                      <div class="mb-2 flex items-center justify-between">
                        <span class="text-xs font-semibold uppercase tracking-wider text-slate-500">纹理库</span>
                        <span class="text-[11px] text-slate-400">{choices().length} 个可选</span>
                      </div>
                      <Show
                        when={choices().length}
                        fallback={
                          <div class="rounded border border-dashed border-slate-200 bg-slate-50 px-3 py-5 text-center text-xs text-slate-400">
                            纹理库暂无此类型资源
                          </div>
                        }
                      >
                        <div class="grid max-h-44 grid-cols-3 gap-2 overflow-y-auto pr-1">
                          <For each={choices()}>
                            {(texture) => (
                              <button
                                type="button"
                                class={`overflow-hidden rounded border bg-white transition-colors ${chosen() === texture.uuid ? 'border-primary ring-2 ring-primary/20' : 'border-slate-200 hover:bg-slate-50'}`}
                                onClick={() => chooseTexture(texture.uuid)}
                              >
                                <div class="flex h-16 items-center justify-center bg-slate-100">
                                  <img src={texture.url} alt={texture.uuid} class="h-12 w-12 object-contain" style="image-rendering:pixelated" />
                                </div>
                                <div class="truncate px-1.5 py-1 font-mono text-[10px] text-slate-400">{texture.uuid.slice(0, 8)}</div>
                              </button>
                            )}
                          </For>
                        </div>
