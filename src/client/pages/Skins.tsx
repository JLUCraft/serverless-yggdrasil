...
import { api, forgetLogin, authRejected, type Shape, type Texture, type TextureKind, type User } from '../lib/api';
      ...
      <Show when={!loading() && member()}>
        ...
        <div class="space-y-6">
          <div class="border-b border-base-300 pb-4">
            <h1 class="text-2xl font-bold tracking-tight text-base-content">纹理库</h1>
            <p class="mt-0.5 text-sm text-base-content/60">管理皮肤与披风资源，档案绑定请在档案配置中完成。</p>
          </div>
          ...
          <div class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_280px] items-start">
            <div class="space-y-6">
              <section>
                <h2 class="mb-3 text-xs font-bold uppercase tracking-widest text-base-content/50">已有纹理</h2>
                <Show
                  when={textures().length}
                  fallback={
                    <div class="rounded border border-dashed border-base-300 bg-base-200 px-6 py-10 text-center text-sm text-base-content/50">
                      暂无纹理，请先上传皮肤或披风。
                    </div>
                  }
                >
                    ...
                    <For each={textures()}>
                      {(texture) => (
                        <article
                          role="button"
                          tabIndex={0}
                          class={`glass-panel overflow-hidden text-left transition-all ${selected()?.uuid === texture.uuid ? 'ring-2 ring-primary/40' : 'hover:bg-base-200'}`}
                        ...
                        >
                          <div class="flex h-20 items-center justify-center bg-base-200">
                            <img src={texture.url} alt={texture.uuid} class="h-16 w-16 object-contain" style="image-rendering:pixelated" />
                          </div>
                          <div class="p-2.5">
                            <div class="flex items-center justify-between gap-1.5">
                              <span class={`rounded px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide ${texture.type === 'skin' ? 'bg-primary/10 text-primary' : 'bg-base-300 text-base-content/70'}`}>
                                {texture.type}
                              </span>
                              <span class="truncate font-mono text-[10px] text-base-content/40">{texture.uuid.slice(0, 12)}</span>
                            </div>
                            <button
                              type="button"
                              class="mt-2 text-[11px] font-bold text-error disabled:text-base-content/40"
              ...
              <section class="glass-panel p-5 space-y-4">
                <div>
                  <h2 class="font-bold text-base-content">上传新纹理</h2>
                  <p class="mt-0.5 text-xs text-base-content/60">选择 PNG 文件后在右侧实时预览，确认后加入纹理库。</p>
                </div>

                <div>
                  <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-base-content/60">纹理类型</label>
                  <div class="grid grid-cols-2 gap-2">
                    {(['skin', 'cape'] as const).map((texture) => (
                      <button
                        type="button"
                        class={`rounded border py-2 text-sm font-semibold transition-colors ${uploadKind() === texture ? 'border-primary/25 bg-primary/10 text-primary' : 'border-base-300 bg-base-100 text-base-content/70 hover:bg-base-200'}`}
            ...
            <div class="space-y-3">
              <h2 class="text-xs font-bold uppercase tracking-widest text-base-content/50">实时预览</h2>
              ...
              <Show when={selected()?.type === 'skin' || (draft() && uploadKind() === 'skin')}>
                <div>
                  <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-base-content/60">预览模型</label>
                  <div class="grid grid-cols-2 gap-2">
                    {(['default', 'slim'] as const).map((shape) => (
                      <button
                        type="button"
                        class={`rounded border py-1.5 text-xs font-semibold transition-colors ${previewShape() === shape ? 'border-primary/25 bg-primary/10 text-primary' : 'border-base-300 bg-base-100 text-base-content/70 hover:bg-base-200'}`}
