...
app.post('/upload', authMiddleware, async (c) => {
  ...
  if (type === 'skin') {
    ...
    if (dims.width % 64 !== 0 || dims.height % 32 !== 0 || (ratio !== 2 && ratio !== 1)) {
      return error(`Skin must have width multiple of 64, height multiple of 32, and aspect ratio 1:1 or 2:1, got ${dims.width}x${dims.height}`, 400);
    }
  } else if (type === 'cape') {
    const ratio = dims.width / dims.height;
    if (ratio !== 2) {
      return error(`Cape must have 2:1 aspect ratio, got ${dims.width}x${dims.height}`, 400);
    }
  }
